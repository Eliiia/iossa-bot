...
export async function run(client: Client, interaction: Interaction) {
    ...
    if (interaction.isCommand()) {
        ...
        console.log(
            `${interaction.user.tag} ran /${interaction.commandName}; ${
    ...
    if (interaction.isButton()) {
        interaction.guild?.channels
            .create(`ticket-${Date.now() - Number(process.env.EPOCH)}`, {
                type: "GUILD_TEXT",
                parent: process.env.TICKETCATEGORYID,
                permissionOverwrites: [
                    {
                        id: process.env.MODMAILROLEID as string,
                        allow: ["VIEW_CHANNEL"],
                    },
            ...
            .then((channel) => {
                interaction.reply({
                    content: `Created ticket <#${channel.id}>!`,
                    ephemeral: true,
                });
            });
    } else return;
