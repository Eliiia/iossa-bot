...
const commands = {
    main: [
        new SlashCommandBuilder().setName("close").setDescription("Closes current ticket."),
    ].map(command => command.toJSON()),
    dev: [
        new SlashCommandBuilder().setName("eval").setDescription("Runs custom code. (dev-only)"),
    ].map(command => command.toJSON())
}
